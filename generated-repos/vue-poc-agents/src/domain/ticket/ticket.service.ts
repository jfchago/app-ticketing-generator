...
export class TicketService {
  ...
  async getAll(): Promise<Ticket[]> {
    try {
      await this.hooks?.beforeGetAll?.();
    } catch {}
    const result = await this.repository.getAll();
    try {
      await this.hooks?.afterGetAll?.(result);
    } catch {}
    return result;
  }
  ...
  async updateStatus(ticketId: string, status: TicketStatus): Promise<Ticket> {
    try {
      await this.hooks?.beforeUpdateStatus?.(ticketId, status);
    } catch {}

    const result = await this.repository.updateStatus(ticketId, status);
    try {
      await this.hooks?.afterUpdateStatus?.(result);
    } catch {}
    return result;
  }
  ...
  async updatePriority(ticketId: string, priority: TicketPriority): Promise<Ticket> {
    try {
      await this.hooks?.beforeUpdatePriority?.(ticketId, priority);
    } catch {}

    const result = await this.repository.updatePriority(ticketId, priority);
    try {
      await this.hooks?.afterUpdatePriority?.(result);
    } catch {}
    return result;
  }
  ...
  async assignUser(ticketId: string, userId: string): Promise<Ticket> {
    try {
      await this.hooks?.beforeAssignUser?.(ticketId, userId);
    } catch {}

    const result = await this.repository.assignUser(ticketId, userId);
    try {
      await this.hooks?.afterAssignUser?.(result);
    } catch {}
    return result;
  }
  ...
  async unassignUser(ticketId: string): Promise<void> {
    try {
      await this.hooks?.beforeUnassignUser?.(ticketId);
    } catch {}

    await this.repository.unassignUser(ticketId);
    try {
      await this.hooks?.afterUnassignUser?.();
    } catch {}
  }

  async addComment(ticketId: string, text: string): Promise<Comment> {
    try {
      await this.hooks?.beforeAddComment?.(ticketId, text);
    } catch {}

    const result = await this.repository.addComment(ticketId, text);
    try {
      await this.hooks?.afterAddComment?.(result);
    } catch {}
    return result;
  }
}
