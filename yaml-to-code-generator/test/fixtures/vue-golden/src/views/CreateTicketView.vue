...
async function handleSubmit(data: Partial<Ticket>) {
  try {
    ...
    const result = await store.create(data.title ?? '', data.description ?? '', data.priority ?? '');
