  ...
  actions: {
    ...
    async getAll() {
      this.loading = true; this.error = null;
        try { this.tickets = await service.getAll(); }
        catch (e) { this.error = e instanceof Error ? e.message : 'Error loading tickets'; }
    ...
    async getById(id: string) {
      this.loading = true; this.error = null;
        ...
        catch (e) { this.error = e instanceof Error ? e.message : 'Error loading ticket'; }
        finally { this.loading = false; }
    },


    async create(data: Omit<Ticket, 'id' | 'createdAt'>) {
      if (!(data.title.trim().length > 0)) { this.error = 'El título no puede estar vacío'; throw new Error('El título no puede estar vacío'); }
        this.loading = true; this.error = null;
        try { const created = await service.create(data);
          this.tickets.push(created); return created;
        } catch (e) { this.error = e instanceof Error ? e.message : 'Error creating ticket'; throw e; }
        finally { this.loading = false; }
    },
    ...
    async updateStatus(ticketId: string, status: TicketStatus) {
      this.error = null;
        try { const ticket = this.tickets.find(t => t.id === ticketId);
          if (!ticket) { this.error = 'Ticket not found'; return; }
          if (!(status !== 'CLOSED' || ticket.comments.length > 0)) { this.error = 'Requiere al menos un comentario para cerrar'; return; }
    ...
    async updatePriority(ticketId: string, priority: TicketPriority) {
      this.error = null;
        try { const ticket = this.tickets.find(t => t.id === ticketId);
          if (!ticket) { this.error = 'Ticket not found'; return; }
          if (!(priority !== 'LOW' || ticket.priority !== 'LOW')) { this.error = 'La prioridad ya es LOW'; return; }
    ...
    async getHistory(entityId: string) {
      this.error = null;
        ...
        if (this.history.entries.length === 0) {
          this.history.loadingInitial = true;
          try {
            const page = await service.getHistory(entityId);
            this.history.entries = page.items;
            this.history.cursor = page.nextCursor;
            this.history.hasMore = page.hasMore;
          } catch (e) { this.history.error = e instanceof Error ? e.message : 'Failed to load history'; }
          finally { this.history.loadingInitial = false; }
        }
    },


    async fetchNextPage(entityId: string) {
      this.error = null;
if (!this.history) this.history = { entries: [], cursor: null, hasMore: false, loadingInitial: false, loadingMore: false, error: null };
...
try {
  ...
  this.history.hasMore = page.hasMore;
    ...
    async clearHistory() {
      this.history = { entries: [], cursor: null, hasMore: false, loadingInitial: false, loadingMore: false, error: null };
    },
