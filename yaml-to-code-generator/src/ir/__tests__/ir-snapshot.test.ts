...
const GOLDEN_PATH = path.resolve(_testDir, '../../..', 'test/fixtures/ir-golden-master.json');
...
describe('IR Snapshot', () => {
  ...
  it('Ticket entity has correct snapshot properties', () => {
    ...
    expect(ticket.useCaseNames.sort()).toEqual([
      'add_comment', 'assign_user', 'create', 'get_all', 'get_by_id',
      'unassign_user', 'update_priority', 'update_status',
    ]);
  ...
  it('has correct enums', () => {
    ...
    expect(priority.values.map(v => v.name)).toEqual(['LOW', 'MEDIUM', 'HIGH', 'URGENT']);
  });
  ...
  it('IR use cases have categories', () => {
    ...
    for (const uc of ticket.useCases) {
      if (uc.name === 'get_all' || uc.name === 'get_by_id' || uc.name === 'load_users') {
