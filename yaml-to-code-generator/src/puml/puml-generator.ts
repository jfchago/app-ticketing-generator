...
export function generatePumlClassDiagram(ir: IR): string {
  ...
  for (const entity of ir.entities) {
    const stereo = entity.stereotype === 'aggregate_root'
      ? '<<aggregate_root>>'
      : `<<${entity.stereotype}>>`;
    ...
    for (const attr of entity.attributes) {
      ...
      lines.push(`  ${visibility} ${attr.name}${nullable} : ${attr.type}${annotationStr}`);
    }
  ...
  for (const entity of ir.entities) {
    for (const rel of entity.relationships) {
      if (rel.type === 'many_to_one') {
        ...
        lines.push(`${entity.namePascal} ${sourceCard} --> ${targetCard} ${rel.targetPascal} : ${rel.name}`);
      } else if (rel.type === 'one_to_many') {
        const sourceCard = '1';
        const targetCard = '"0..*"';
        lines.push(`${entity.namePascal} "${sourceCard}" --> "${targetCard}" ${rel.targetPascal} : ${rel.name}`);
