...
import type { RawSpec, ValidatedEntity, ValidatedAttribute, ValidatedRelationship } from '../validator/schema-validator.js';
...
function extractEntities(content: string): PumlEntity[] {
  ...
  while (i < lines.length) {
    ...
    while (j < lines.length && openBraces > 0) {
      ...
      const attrMatch = bodyLine.match(/^([-+#~])\s+(\w+)\s*:\s*(\w+)(?:\s*\{(\w+)\})?(?:\s*<<(\w+)>>)?/);
      if (attrMatch) {
...
function buildSpec(
  ...
  const entityDefs: ValidatedEntity[] = entities.map((e) => ({
    ...
    attributes: e.attributes.map((a): ValidatedAttribute => ({
      name: a.name,
      type: a.type,
