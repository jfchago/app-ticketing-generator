...
describe('compareVersions', () => {
  ...
  it('returns negative when a < b', () => {
    ...
    expect(
      compareVersions({ major: 1, minor: 1, patch: 0 }, { major: 1, minor: 1, patch: 1 }),
    ).toBeLessThan(0);
  });
...
describe('isVersionOlder', () => {
  it('detects older versions', () => {
    expect(isVersionOlder({ major: 1, minor: 0, patch: 0 }, { major: 2, minor: 0, patch: 0 })).toBe(
      true,
    );
  });

  it('returns false for same or newer versions', () => {
    expect(isVersionOlder({ major: 2, minor: 0, patch: 0 }, { major: 1, minor: 0, patch: 0 })).toBe(
      false,
    );
    expect(isVersionOlder({ major: 1, minor: 0, patch: 0 }, { major: 1, minor: 0, patch: 0 })).toBe(
      false,
    );
  });
});
...
describe('versionsEqual', () => {
  it('returns true for equal versions', () => {
    expect(versionsEqual({ major: 1, minor: 0, patch: 0 }, { major: 1, minor: 0, patch: 0 })).toBe(
      true,
    );
  });

  it('returns false for different versions', () => {
    expect(versionsEqual({ major: 1, minor: 0, patch: 0 }, { major: 1, minor: 0, patch: 1 })).toBe(
      false,
    );
  });
});
