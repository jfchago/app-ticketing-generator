...
describe("Spring Golden Master", () => {
  ...
  it("spring gen model has all entities", () => {
    expect(Object.keys(springGen.entities).sort()).toEqual([
      "Comment",
      "Ticket",
      "User",
    ]);
  ...
  it("generated golden master files match", () => {
    ...
    expect(goldenPaths).toContain(
      "src/main/java/com/helpdesk/service/TicketService.java",
    );
    expect(goldenPaths).toContain(
      "src/main/java/com/helpdesk/controller/TicketController.java",
    );
    expect(goldenPaths).toContain(
      "src/main/java/com/helpdesk/entity/Ticket.java",
    );
    expect(goldenPaths).toContain(
      "src/main/java/com/helpdesk/repository/TicketRepository.java",
    );
    expect(goldenPaths).toContain(
      "src/main/java/com/helpdesk/dto/TicketDTO.java",
    );
    expect(goldenPaths).toContain(
      "src/main/java/com/helpdesk/dto/TicketMapper.java",
    );
    expect(goldenPaths).toContain(
      "src/main/java/com/helpdesk/entity/TicketStatus.java",
    );
    expect(goldenPaths).toContain(
      "src/main/java/com/helpdesk/entity/TicketPriority.java",
    );
  ...
  it("golden master files contain no domain conditionals", () => {
    ...
    for (const [fileRelPath, content] of golden) {
      expect(
        content,
        `File ${fileRelPath} contains Ticket name check`,
      ).not.toContain("entity.name === 'Ticket'");
