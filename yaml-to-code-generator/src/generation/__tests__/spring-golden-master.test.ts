...
describe('Spring Golden Master', () => {
  ...
  it('golden master files contain no domain conditionals', () => {
    ...
    for (const [fileRelPath, content] of golden) {
      expect(content, `File ${fileRelPath} contains Ticket name check`).not.toContain("entity.name === 'Ticket'");
      expect(content, `File ${fileRelPath} contains uc.name check`).not.toMatch(/uc\.name === '(get_all|get_by_id|create|update_status|update_priority|assign_user|unassign_user|add_comment|load_users)'/);
    }
  });
});
