...
describe("Vue Golden Master", () => {
  ...
  it("vue gen model has all entities", () => {
    expect(Object.keys(vueGen.entities).sort()).toEqual([
      "Comment",
      "Ticket",
      "User",
    ]);
  ...
  it("generated golden master files match", () => {
    ...
    expect(goldenPaths).toContain(
      "src/infrastructure/repositories/ticket.repository.impl.ts",
    );
  ...
  it("golden master files contain no domain conditionals", () => {
    ...
    for (const [fileRelPath, content] of golden) {
      expect(
        content,
        `File ${fileRelPath} contains domain conditional`,
      ).not.toContain("entity.name === 'Ticket'");
      expect(
        content,
        `File ${fileRelPath} contains domain conditional`,
      ).not.toContain("uc.name === 'get_all'");
      expect(
        content,
        `File ${fileRelPath} contains domain conditional`,
      ).not.toContain("uc.name === 'get_by_id'");
      expect(
        content,
        `File ${fileRelPath} contains domain conditional`,
      ).not.toContain("uc.name === 'create'");
      expect(
        content,
        `File ${fileRelPath} contains domain conditional`,
      ).not.toContain("uc.name === 'update_status'");
      expect(
        content,
        `File ${fileRelPath} contains domain conditional`,
      ).not.toContain("uc.name === 'update_priority'");
      expect(
        content,
        `File ${fileRelPath} contains domain conditional`,
      ).not.toContain("uc.name === 'assign_user'");
      expect(
        content,
        `File ${fileRelPath} contains domain conditional`,
      ).not.toContain("uc.name === 'unassign_user'");
      expect(
        content,
        `File ${fileRelPath} contains domain conditional`,
      ).not.toContain("uc.name === 'add_comment'");
      expect(
        content,
        `File ${fileRelPath} contains domain conditional`,
      ).not.toContain("uc.name === 'load_users'");
    }
  });
});
