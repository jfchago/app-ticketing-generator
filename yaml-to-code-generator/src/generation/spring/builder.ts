...
function buildSpringEntity(entity: EntityDef, ir: IR): SpringGeneratedEntity {
  ...
  return {
    ...
    emitsEvents:
      (ir.events?.length ?? 0) > 0 || (entity.entityEvents?.length ?? 0) > 0,
...
function buildAttributeTypes(
  entity: EntityDef,
): Record<string, SpringAttributeType> {
...
function findCommentEntity(entity: EntityDef, ir: IR): EntityDef | undefined {
  const commentRel = entity.relationships.find(
    (r) =>
      ...
      ir.entities.some(
        (e) =>
          e.name === r.target && e.attributes.some((a) => a.name === "text"),
      ),
  );
  return commentRel
...
function buildServiceMethod(
  ...
  const priorityAttr = entity.attributes.find(
...
function buildEventPublishBody(
  ir: IR,
  entity: EntityDef,
  _action: string,
): string {
  ...
  const matching = events.filter((ev) =>
    ev.namePascal.startsWith(entity.namePascal),
  );
...
function categoryIsCreate(uc: UseCaseDef): boolean {
  return (
    uc.name === "create" ||
    (uc.httpMethod === "POST" && !uc.needsId && uc.needsPayload)
  );
}
