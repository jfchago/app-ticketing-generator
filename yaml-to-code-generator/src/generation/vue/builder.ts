...
function buildVueEntity(entity: EntityDef, ir: IR): VueGeneratedEntity {
  ...
  const hasCommentSupport =
    !!commentRel && entity.useCases.some((uc) => uc.name === "add_comment");
  const commentEntity = hasCommentSupport
  ...
  return {
    ...
    commentEntity: commentEntity
      ? {
          namePascal: commentEntity.namePascal,
...
function findCommentRelationship(
  entity: EntityDef,
  ir: IR,
  ...
  const textEntityNames = new Set(
    ir.entities
      .filter((e) =>
        e.attributes.some((a) => a.name === "text" && a.type === "String"),
      )
      .map((e) => e.name),
  );
...
function mapToTSType(attr: AttributeDef): string {
  ...
  let tsType =
    primitiveMap[attr.type] ??
    (attr.isEnum || attr.isEntityRef ? attr.type : "unknown");
...
function computeFieldRole(attr: AttributeDef): string {
  ...
  if (
    ["createdat", "updatedat", "datetime", "date", "timestamp"].includes(name)
  )
    return "timestamp";
  if (name === "title" || name === "description" || name === "text")
    return "text";
  return "field";
}
...
function isCommentTarget(entity: EntityDef, ir: IR): boolean {
  return (
    ir.entities.some((outer) =>
      outer.relationships.some(
        (r) => r.type === "one_to_many" && r.target === entity.name,
      ),
...
function buildComponentFlags(entity: EntityDef, ir: IR): VueComponentFlags {
  return {
    ...
    shouldRenderForm:
      entity.useCases.some((uc) => uc.name === "create") ||
      entity.useCases.some((uc) =>
        ["update", "update_status", "update_priority"].includes(uc.name),
      ),
    shouldRenderStatusBadge: entity.attributes.some(
      (a) => a.name === "status" && a.isEnum,
    ),
    shouldRenderPriorityBadge: entity.attributes.some(
      (a) => a.name === "priority" && a.isEnum,
    ),
    shouldRenderAssigneeBadge: entity.relationships.some(
      (r) => r.name === "assignee",
    ),
  };
}
...
function buildApiPath(
  ...
  uc: UseCaseDef,
  paramNames: string[],
  parentEntityKebab?: string,
): string {
  const effectiveKebab =
    uc.name === "add_comment" && parentEntityKebab
      ? parentEntityKebab
      : entityKebab;
  const base = `/${effectiveKebab}s`;
  const idVar = paramNames.length > 0 ? paramNames[0] : "id";
  if (uc.pathSuffix)
    return `${base}${uc.pathSuffix.replace(/\{id\}/g, `\${${idVar}}`)}`;
...
function buildResponseType(
  ...
  if (uc.name === "add_comment" && commentEntity)
    return commentEntity.namePascal;
  if (!uc.needsPayload && uc.needsId && uc.httpMethod === "DELETE")
    return "void";
...
function buildParamTypes(entity: EntityDef, uc: UseCaseDef): string[] {
  ...
  if (uc.name === "create")
    return [`Omit<${entity.namePascal}, 'id' | 'createdAt'>`];
  if (uc.needsId && uc.needsPayload) {
    let valType = "string";
    if (uc.name === "update_status") {
      valType =
        entity.attributes.find((a) => a.name === "status")?.type ?? "string";
    } else if (uc.name === "update_priority") {
      valType =
        entity.attributes.find((a) => a.name === "priority")?.type ?? "string";
    } else {
      valType =
        entity.attributes.find((a) => a.isEnum && !a.primary)?.type ?? "string";
    }
...
function buildMethodParams(entity: EntityDef, uc: UseCaseDef): string {
  ...
  if (uc.name === "add_comment")
    return `${entity.nameCamel}Id: string, text: string`;
  if (uc.name === "create")
    return `data: Omit<${entity.namePascal}, 'id' | 'createdAt'>`;
  if (uc.needsId && uc.needsPayload) {
    let valType = "string";
    if (uc.name === "update_status") {
      valType =
  ...
  for (const uc of entity.useCases) {
    ...
    result[uc.name] = {
      ...
      storeAction: {
        ...
        ruleChecks: (entity.entityRules ?? [])
          .filter((r) => r.on === uc.name)
          .map((r) => ({
            nameCamel: r.nameCamel,
            namePascal: r.namePascal,
            guardExpression: r.guard
              ? r.guard.replace(/\bentity\b/g, entity.nameCamel)
              : "",
            message: r.message,
            on: r.on,
          })),
...
function buildStoreActionBody(
  ...
  const hasTransitions =
    uc.actionCategory === "updateStatus" &&
    entity.transitions &&
    Object.keys(entity.transitions).length > 0;
  ...
  if (actionCategory === "read") {
    ...
    bodyParts.push(
      `try { this.${stateList} = await service.${uc.methodName}(); }`,
    );
    bodyParts.push(
      `catch (e) { this.error = e instanceof Error ? e.message : 'Error loading ${entityVar}s'; }`,
    );
    ...
    bodyParts.push(`this.loading = true; this.error = null;`);
    bodyParts.push(
      `try { this.current = await service.${uc.methodName}(${uc.storeAction.paramNames.join(", ")}); }`,
    );
    bodyParts.push(
      `catch (e) { this.error = e instanceof Error ? e.message : 'Error loading ${entityVar}'; }`,
    );
    bodyParts.push(`finally { this.loading = false; }`);
    ...
    bodyParts.push(`this.loading = true; this.error = null;`);
    bodyParts.push(
      `try { const created = await service.${uc.methodName}(${uc.storeAction.paramNames.join(", ")});`,
    );
    bodyParts.push(`  this.${stateList}.push(created); return created;`);
    bodyParts.push(
      `} catch (e) { this.error = e instanceof Error ? e.message : 'Error creating ${entityVar}'; throw e; }`,
    );
    bodyParts.push(`finally { this.loading = false; }`);
  } else if (actionCategory === "updateStatus") {
    bodyParts.push(`this.error = null;`);
    bodyParts.push(
      `try { const ${entityVar} = this.${stateList}.find(t => t.id === ${idParam});`,
    );
    bodyParts.push(
      `  if (!${entityVar}) { this.error = '${entity.namePascal} not found'; return; }`,
    );
    bodyParts.push(...buildRuleCheckLines(actionCategory).map((l) => `  ${l}`));
    if (hasTransitions) {
      bodyParts.push(`  const statusAttr = ${entityVar}.status as string;`);
      bodyParts.push(
        `  if (!VALID_TRANSITIONS[statusAttr]?.includes(${valParam})) {`,
      );
      bodyParts.push(
        `    this.error = \`Invalid transition from \${statusAttr} to \${${valParam}}\`;`,
      );
      bodyParts.push(`    return;`);
      bodyParts.push(`  }`);
    }
    bodyParts.push(
      `  await service.${uc.methodName}(${uc.storeAction.paramNames.join(", ")});`,
    );
    bodyParts.push(`  ${entityVar}.status = ${valParam};`);
    bodyParts.push(
      `  if (this.current?.id === ${idParam}) this.current.status = ${valParam};`,
    );
    bodyParts.push(
      `} catch (e) { this.error = e instanceof Error ? e.message : 'Error updating status'; throw e; }`,
    );
    ...
    bodyParts.push(
      `  if (this.current?.id === ${idParam}) this.current.priority = ${valParam};`,
    );
    bodyParts.push(
      `} catch (e) { this.error = e instanceof Error ? e.message : 'Error updating priority'; throw e; }`,
    );
    ...
    bodyParts.push(
      `  if (this.current?.id === ${idParam}) this.current.assigneeId = ${valParam};`,
    );
    bodyParts.push(
      `} catch (e) { this.error = e instanceof Error ? e.message : 'Error assigning user'; throw e; }`,
    );
    ...
    bodyParts.push(
      `  if (this.current?.id === ${idParam}) this.current.assigneeId = null;`,
    );
    bodyParts.push(
      `} catch (e) { this.error = e instanceof Error ? e.message : 'Error unassigning user'; throw e; }`,
    );
    ...
    bodyParts.push(`this.error = null;`);
    bodyParts.push(
      `try { await service.${uc.methodName}(${uc.storeAction.paramNames.join(", ")}); }`,
    );
    bodyParts.push(
      `catch (e) { this.error = e instanceof Error ? e.message : 'Error in ${uc.methodName}'; throw e; }`,
    );
  }
...
function deriveStoreActions(
  ...
  return Object.values(useCases).map((uc) => {
    ...
    const body = buildStoreActionBody(
      entity,
      uc,
      actionCategory,
      stateList,
