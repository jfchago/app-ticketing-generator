...
function computeFieldRole(attr: AttributeDef): string {
  ...
  if (['createdat', 'updatedat', 'datetime', 'date', 'timestamp'].includes(name))
    return 'timestamp';
...
function buildApiPath(
  ...
  const effectiveKebab =
    uc.name === 'add_comment' && parentEntityKebab ? parentEntityKebab : entityKebab;
    ...
    result[uc.name] = {
      ...
      apiCall: {
        path: buildApiPath(
          ...
          ir.entities.find((e) =>
            e.relationships.some((r) => r.type === 'one_to_many' && r.target === entity.name),
          )?.nameKebab,
        ),
