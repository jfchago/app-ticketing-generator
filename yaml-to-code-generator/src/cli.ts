...
import { generate, validateOnly, pumlToYaml, yamlToPuml, yamlToDiagrams, inspectStage } from './generator/orchestrator.js';
...
program
  ...
  .description('Generate all PlantUML diagrams from YAML (class, state, activity, sequence, use case)')
  .requiredOption('-s, --spec <path>', 'Path to the YAML specification file')
...
program
  ...
  .requiredOption('-s, --spec <path>', 'Path to the YAML specification file')
  .requiredOption('--stage <name>', 'Pipeline stage to inspect (raw, validated, semantic, ir, vue-model, spring-model)')
  .action((options) => {
    try {
