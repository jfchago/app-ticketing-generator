...
program
  ...
  .requiredOption(
    "-t, --target <name>",
    "Target template pack (vue, spring, diagrams)",
  )
  ...
  .option(
    "--build-tool <tool>",
    "Build tool for Spring target: gradle (default) or maven",
  )
  .option(
    "--strict-behavior",
    "Enable strict behavior block validation (parse + cross-reference)",
  )
...
program
  ...
  .description(
    "Generate a PlantUML class diagram from YAML (legacy, single file)",
  )
  .requiredOption("-s, --spec <path>", "Path to the YAML specification file")
...
program
  .command("inspect")
  .description(
    "Inspect a pipeline stage and dump its JSON representation to stdout",
  )
  .requiredOption("-s, --spec <path>", "Path to the YAML specification file")
  .requiredOption(
    "--stage <name>",
