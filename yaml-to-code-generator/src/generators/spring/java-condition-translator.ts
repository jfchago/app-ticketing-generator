...
export function toJavaCondition(condition: string, entity?: EntityDef): string {
  ...
  r = r.replace(/(\w+)\.length\s*(>|<|>=|<=|!==|!=|==)/g, (_, field, op) => {
    if (
      entity?.relationships?.some(
        (rel) => rel.name === field && rel.type === "one_to_many",
      )
    ) {
  ...
  r = r.replace(
    /entity\.(\w+)/g,
    (_, field: string) =>
      `entity.get${field.charAt(0).toUpperCase() + field.slice(1)}()`,
  );
  ...
  r = r.replace(
    /context\.(\w+)/g,
    'context.getExtendedState().get("$1", String.class)',
  );
