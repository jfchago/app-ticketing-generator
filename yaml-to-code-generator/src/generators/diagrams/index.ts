...
const _diagramTemplates =
  dirname(fileURLToPath(import.meta.url)) + "/templates";
    ...
    if (entity.useCases.length > 0) {
      ...
      for (const uc of entity.useCases) {
        const retType =
          entity.namePascal +
          "DTO" +
          (["get_all", "load_users"].includes(uc.name) ? "[]" : "");
...
function generateUseCaseDiagram(ir: any): string {
  const workflowActors = new Set(
    (ir.workflows ?? []).flatMap((w: any) =>
      (w.participants ?? []).map((p: any) => p.name),
    ),
  );
  ...
  for (const entity of ir.entities) {
    ...
    for (const uc of entity.useCases) {
      ...
      for (const actor of actors) {
        lines.push(
          "  " + actor + " --> UC_" + entity.namePascal + "_" + uc.namePascal,
        );
      }
    }
  }
  ...
  for (const step of wf.steps) {
    if (step.type === "gateway") {
      ...
      for (const t of step.transitions.filter((ti) => !ti.isDefault)) {
        ...
        for (const action of step.actions) {
          const sender =
            (action.params as any)?.actor || step.actorName || "System";
          lines.push(
            "  " +
              sender +
              " -> " +
