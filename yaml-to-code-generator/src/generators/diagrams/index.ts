...
export class DiagramGenerator extends BaseGenerator {
  ...
  writing(): void {
    ...
    this.#tryRenderOrInline(
      'class-diagram.puml.ejs',
      `${baseName}-class-diagram.puml`,
      {},
    ...
    for (const entity of ir.entities) {
      for (const uc of entity.useCases) {
        ...
        try { mkdirSync(this.destinationPath(outDir), { recursive: true }); } catch {}
        this.renderEjs(
...
function generateClassDiagram(ir: any): string {
  ...
  for (const entity of ir.entities) {
    ...
    lines.push('class ' + entity.namePascal + ' << (S,' + stereoColor + ') ' + stereoLabel + ' >> {');
    ...
    if (entity.useCases.length > 0) {
      lines.push('  --');
      for (const uc of entity.useCases) {
        const retType = entity.namePascal + 'DTO' + (['get_all', 'load_users'].includes(uc.name) ? '[]' : '');
  ...
  for (const entity of ir.entities) {
    for (const rel of entity.relationships) {
      ...
      lines.push(entity.namePascal + arrow + '"' + rel.targetCardinality + '" ' + rel.targetPascal + ' : ' + rel.name);
    }
  }
...
function generateUseCaseDiagram(ir: any): string {
  const workflowActors = new Set((ir.workflows ?? []).flatMap((w: any) => (w.participants ?? []).map((p: any) => p.name)));
...
function generateActivityDiagram(wf: WorkflowDef): string {
  ...
  for (const step of wf.steps) {
    ...
    if (step.type === 'gateway') {
      ...
      lines.push(':⏱ ' + (step.label || step.id) + (step.timerExpression ? ' (' + step.timerExpression + ')' : '') + ';');
      ...
      for (const t of step.transitions.filter(ti => !ti.isDefault)) {
        lines.push('  -> ' + (t.condition ? '[' + t.condition + '] ' : '') + ': ' + (t.label || t.targetStep) + ';');
    ...
    if (step.type === 'gateway') {
      ...
      for (const t of step.transitions.filter(ti => !ti.isDefault)) {
        ...
        for (const action of step.actions) {
          ...
          lines.push('  ' + sender + ' -> ' + (sender === 'System' ? (action.target || 'System') : 'System') + ' : ' + ((action.params as any)?.verb || action.type));
        }
        lines.push('  end');
      }
      const defaultT = step.transitions.find(t => t.isDefault);
      if (defaultT) {
        ...
        for (const action of step.actions) {
          const sender = (action.params as any)?.actor || step.actorName || 'System';
          lines.push('  ' + sender + ' -> ' + (action.target || 'System') + ' : ' + ((action.params as any)?.verb || action.type));
        }
        lines.push('  end');
      }
      lines.push('end');
