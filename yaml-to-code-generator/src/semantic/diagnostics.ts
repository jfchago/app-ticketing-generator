...
export function formatDiagnostics(diagnostics: SemanticDiagnostic[]): string {
  ...
  const lines: string[] = [];
  lines.push(
    `   ${diagnostics.length} diagnostic(s): ${bySeverity.error.length} error(s), ${bySeverity.warning.length} warning(s), ${bySeverity.info.length} info`,
  );
  for (const d of diagnostics) {
    const icon = d.severity === 'error' ? '✖' : d.severity === 'warning' ? '⚠' : 'ℹ';
    const loc = d.location
      ? ` [${d.location.kind}:${d.location.name}${d.location.parent ? ` in ${d.location.parent}` : ''}]`
      : '';
...
export class SemanticDiagnosticsError extends Error {
  ...
  constructor(diagnostics: SemanticDiagnostic[]) {
    ...
    super(
      `Semantic validation failed with ${errors.length} error(s):\n${errors.map((e) => `  - [${e.code}] ${e.message}`).join('\n')}`,
    );
