...
describe("Resolver — Symbol resolution", () => {
  ...
  it("resolves use cases with categories", () => {
    ...
    const updateStatus = ticket.useCases.find(
      (u) => u.name === "update_status",
    )!;
...
describe("buildSemanticModel — Integration", () => {
  ...
  it("buildSemanticModelOrThrow throws on invalid spec", () => {
    ...
    badSpec.enums = {};
    expect(() => buildSemanticModelOrThrow(badSpec)).toThrow(
      SemanticDiagnosticsError,
    );
  });
...
describe("Semantic Model Snapshot", () => {
  ...
  it("has entity snapshots with key data", () => {
    ...
    const priorityAttr = ticket.attributes.find(
      (a: any) => a.name === "priority",
    );
    expect(priorityAttr?.resolvedType?.ref).toBe("TicketPriority");
    const assigneeRel = ticket.relationships.find(
      (r: any) => r.name === "assignee",
    );
    expect(assigneeRel?.resolvedTarget?.ref).toBe("User");
    const commentsRel = ticket.relationships.find(
      (r: any) => r.name === "comments",
    );
